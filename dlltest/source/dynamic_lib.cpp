...
void Relu(int w, int h, double *input_matrix, double *output_matrix)
{
	for (int i = 0; i < w; i++)																		 // 遍历输入矩阵的行
		for (int j = 0; j < h; j++)																	 // 遍历输入矩阵的列
			output_matrix[i * w + j] = max___(input_matrix[i * w + j], input_matrix[i * w + j] * 0.05); // 计算 ReLU 激活结果
}
...
DLL_API int recongise(const char *path_bmp)
{
	...
	int res = read_file(storage); // 尝试读取网络参数文件
	if (res == FALSE)				// 如果读取失败
	{
		printf("参数包不存在!随机参数\n");
	...
	else if (res == TRUE) // 如果读取成功
	{
		//printf("参数读取成功!\n");
	}
	FILE *fp;
	char sp[200];
	strcpy(sp, path_bmp); // 生成测试样本文件名
	printf("\n打开的文件名:%s", sp);
	fp = fopen(sp, "rb"); // 打开测试样本文件
	if (!fp)
	{
		perror("不能打开文件!\n");
		return -1;
	...
	for (int u = 0; u < 30; u++) // 遍历30行
	{
		...
		for (int j = 0; j < 32; j++) // 遍历32列
		{
			if ((j != 30) && (j != 31)) // 跳过第30和第31列
			{
				data_img[u][y] = l[g]; // 将二值化数据存储到样本数组中
				y++;
			};
			g++;
		}
	...
	forward(&data_img[0][0], storage, data); // 前向传播
